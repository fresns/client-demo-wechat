...
Page({
  ...
  onLoad: async function (options) {
    let requestState = await fresnsConfig('menu_user_query_state');
    let requestQuery = parseUrlParams(await fresnsConfig('menu_user_query_config'));
    ...
    this.setData({
      title: await fresnsConfig('menu_user_title'),
  ...
  loadFresnsPageData: async function () {
    ...
    if (resultRes.code === 0) {
      ...
      let tipType = 'none';
      if (isReachBottom) {
        tipType = this.data.users ? 'page' : 'empty';
      }
