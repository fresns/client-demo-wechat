...
Page({
  ...
  onLoad: async function (options) {
    options.geotags = options.gtid;

    this.setData({
      title: await fresnsConfig('geotag_name'),
      detailType: await fresnsConfig('channel_geotag_detail_type'),
    ...
    const detailRes = await fresnsApi.geotag.detail(options.gtid);

    if (detailRes.code === 0) {
      const geotag = detailRes.data.detail;
      ...
      callPrevPageFunction('onChangegeotag', geotag);
    }
