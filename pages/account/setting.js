...
Page({
        ...
        wx.chooseImage({
            ...
            success: async function (res) {
                ...
                const uploadRes = await Api.editor.editorUpload(tempFilePaths[0], {
                    ...
                    mode: 1,
                    tableId: that.data.user.uid,
                    file: tempFilePaths[0],
                });
                const resultFile = uploadRes.data.files[0];
                const userEditRes = await Api.user.userEdit({
    ...
    modifyUserLoginPassword: async function (e) {
        ...
        const userEditRes = await Api.user.userEdit(params);
        if (userEditRes.code === 0) {
            this.setData({
                currentPassword: '',
                password: '',
    ...
    modifyUserWalletPassword: async function (e) {
        const { modifyWalletPwdType, currentWalletPassword, walletPassword, confirmWalletPassword, verifyCode } =
            this.data;
        let params = null;
        if (!walletPassword) {
            wx.showToast({
