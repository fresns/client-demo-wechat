Component({
    ...
    methods: {
        ...
        _validateShowTime(_value, _mode) {
            ...
            if (_mode === 'dateminute') {
                ...
                secondReg.test(showTimeValue) &&
                    (showTimeValue = showTimeValue.substring(0, showTimeValue.lastIndexOf(':')));
                munithReg.test(showTimeValue) ||
                    (showTimeValue = currentTime.substring(0, currentTime.lastIndexOf(':')));
            } else {
                // yyyy-MM-dd HH:mm:ss
        ...
        selectColumnChangeFn(ev) {
            ...
            let selectValue = Number(rangeList[column][value].substring(0, rangeList[column][value].length - 1));
            if (column === 1) {
                // 改变月份
                const currentYear = Number(
                    rangeList[0][rangeValue[0]].substring(0, rangeList[0][rangeValue[0]].length - 1)
                );
